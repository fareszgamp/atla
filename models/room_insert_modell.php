...
include '../components/mydbal_db.php';
	...
	*/
	function roomsOpen (){
	$mydb = new myDBC();
		$sql = "SELECT * FROM szobak where szoba_level=1 and chek=0";
		$par = array();
		$type = "";
	...
	Állandó zárt szoba
	*/
	function roomsClosed (){
	$mydb = new myDBC();
		$sql = "SELECT * FROM szobak where szoba_level=1 and chek=1";
		$par = array();
		$type = "";
		$tomb = array("sql"=>$sql,"type"=>$type,"param"=>$par);
		return $mydb->resultArray($tomb);
	...
	*/
	function roomsTemp (){
	$mydb = new myDBC();
		$sql = "SELECT * FROM szobak where szoba_level=0";
		$par = array();
		$type = "";
		$tomb = array("sql"=>$sql,"type"=>$type,"param"=>$par);
		return $mydb->resultArray($tomb);
